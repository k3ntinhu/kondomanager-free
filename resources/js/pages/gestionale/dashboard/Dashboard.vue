...
const pageGuides = [
  {
    title: trans('gestionale.dashboard.guides.budget_title'),
    description: trans('gestionale.dashboard.guides.budget_description'),
    icon: Zap,
    colorVariant: 'blue' as const
  },
  {
    title: trans('gestionale.dashboard.guides.alerts_title'),
    description: trans('gestionale.dashboard.guides.alerts_description'),
    icon: ShieldAlert,
    colorVariant: 'amber' as const
  },
  {
    title: trans('gestionale.dashboard.guides.inbox_title'),
    description: trans('gestionale.dashboard.guides.inbox_description'),
    icon: Inbox,
                                            ...
                                            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                                                    ...
                                                    <div class="flex-1">
                                                        ...
                                                        <div class="flex items-center gap-3 mt-2 text-[10px] font-bold uppercase text-slate-400">
                                                            <span v-if="task.type === 'verifica_pagamento'" class="text-amber-500 flex items-center gap-1">
                                                                <span class="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse"></span> {{ trans('gestionale.dashboard.inbox.badges.to_verify') }}
                                                            </span>
                                                            <span v-else-if="task.status === 'expired'" class="text-red-500 flex items-center gap-1">
                                                                <span class="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span> {{ trans('gestionale.dashboard.inbox.badges.expired') }}
                                                            </span>
                                                            <span v-else class="text-emerald-500 flex items-center gap-1">
                                                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> {{ trans('gestionale.dashboard.inbox.badges.todo') }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="shrink-0 flex items-center gap-2">
                                                    ...
                                                    <template v-if="task.type === 'verifica_pagamento'">
                                                        <button @click="openRejectModal(task)" :title="trans('gestionale.dashboard.reject_modal.reject_report')"
                                                        ...
                                                        class="inline-flex items-center justify-center h-7 px-3 text-xs font-bold transition-all rounded-md bg-white border border-slate-200 text-slate-700 shadow-sm hover:border-emerald-300 hover:text-emerald-700 hover:bg-emerald-50 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-emerald-900/30 dark:hover:text-emerald-400">
                                                            {{ trans('gestionale.dashboard.actions.register') }} <ArrowRight class="w-3.5 h-3.5 ml-1.5" />
                                                        </a>
                                                    </template>

                                                    <template v-else>
                                                        <button @click="completeTask(task.id)" :title="trans('gestionale.dashboard.actions.mark_completed')"
                                                        ...
                                                        <a v-if="task.action_url" :href="task.action_url" 
                                                            ...
                                                            {{ trans('gestionale.dashboard.actions.resolve') }} <ArrowRight class="w-3.5 h-3.5 ml-1.5" />
                                                        </a>
                                                    </template>
            ...
            <DialogContent class="sm:max-w-[450px]">
                <DialogHeader>
                    <DialogTitle class="flex items-center gap-2 text-red-600">
                        ...
                        {{ trans('gestionale.dashboard.reject_modal.reject_report') }}
                    </DialogTitle>
                    <DialogDescription>
                        {{ trans('gestionale.dashboard.reject_modal.you_are_about_to_reject') }}
                        <span class="font-bold text-slate-900">{{ taskToReject?.context?.anagrafica_nome || trans('gestionale.dashboard.reject_modal.condomino') }}</span>.
                        <strong> {{ trans('gestionale.dashboard.reject_modal.irreversible_action') }}</strong>
                    </DialogDescription>
                </DialogHeader>
                ...
                <div class="grid gap-4 py-4">
                    <div class="grid gap-2">
                        <Label htmlFor="reason" class="text-slate-900">
                            {{ trans('gestionale.dashboard.reject_modal.reason_visible_to_user') }}
                        </Label>
                        <Textarea 
                            id="reason" 
                            v-model="rejectForm.reason" 
                            :placeholder="trans('gestionale.dashboard.reject_modal.reason_example')" 
                ...
                <DialogFooter>
                    ...
                    >
                        ...
                        {{ trans('gestionale.dashboard.reject_modal.confirm_rejection') }}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
