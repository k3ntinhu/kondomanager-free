...
const formatMoney = (val: any) => new Intl.NumberFormat(undefined, { style: 'currency', currency: 'EUR' }).format(val / 100);
...
const getDateLabel = (dateStr: string | null, status: string) => {
    ...
    if (status === 'expired') {
        ...
        return diffDays === 1 ? trans('dashboard.inbox.yesterday') : trans('dashboard.inbox.days_late', { count: diffDays });
    }
            ...
            <div class="w-full bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden relative min-h-[600px] flex flex-col">
                ...
                <div v-if="isLoading" 
                    ...
                    <div class="flex flex-col items-center gap-3 animate-pulse">
                        ...
                        <span class="text-xs text-slate-400 font-bold tracking-widest uppercase">{{ trans('dashboard.inbox.loading') }}</span>
                    </div>
                </div>
                            ...
                            <div class="col-span-2 flex items-center gap-3">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <span v-if="task.status !== 'expired'" class="text-[11px] text-slate-400 mt-1 capitalize">
                                        {{ new Date(task.date).toLocaleDateString(undefined, { weekday: 'short' }) }}
                                    </span>
                                </div>
                            </div>
            ...
            <DialogContent class="sm:max-w-[450px]">
                ...
                <DialogFooter>
                    <Button variant="outline" @click="isRejectModalOpen = false" :disabled="rejectForm.processing">
                        {{ trans('dashboard.inbox.reject_modal.cancel') }}
                    </Button>
                    <Button 
                        variant="destructive" 
                        @click="confirmReject" 
