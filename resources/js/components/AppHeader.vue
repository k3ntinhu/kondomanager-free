...
const mainNavItems = computed<NavItem[]>(() => [
    ...
    { type: 'link', title: trans('appHeader.condomini'), href: '/condomini', icon: House, permissions: [Permission.VIEW_CONDOMINI] },
    {
        ...
        icon: SquareLibrary,
        children: [
            { type: 'link', title: trans('appHeader.anagrafiche'), icon: UsersRound, href: generatePath('anagrafiche'), permissions: [Permission.VIEW_ANAGRAFICHE] },
    ...
    {
        type: 'link',
        title: trans('appHeader.documenti'),
    ...
    { type: 'link', title: trans('appHeader.agenda'), href: generatePath('eventi'), icon: CalendarClock, permissions: [Permission.VIEW_EVENTS] },
]);
...
<template>
    <div>
                                ...
                                <HoverCardContent align="start" class="w-48 p-2">
                                    <nav class="space-y-1">
                                        <Link
                                            ...
                                            :key="child.title"
                                            :href="child.href"
                                            class="flex items-center px-3 py-2 rounded-md text-sm hover:bg-accent"
                                            :class="isCurrentRoute(child.href) ? 'bg-accent text-accent-foreground' : ''"
                                        >
                ...
                <div class="ml-auto flex items-center gap-2">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger as-child>
                            <Button variant="ghost" size="icon" class="relative rounded-full p-1">
        ...
        <div v-if="props.breadcrumbs.length > 1" class="w-full border-b border-sidebar-border/70 bg-background/50 backdrop-blur-sm">
