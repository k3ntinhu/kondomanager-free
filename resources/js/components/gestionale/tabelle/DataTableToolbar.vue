...
// Props
const props = defineProps<{ table: Table<Tabella> }>();
...
watchDebounced(
  ...
  () => {
    router.get(
      route(generateRoute('gestionale.tabelle.index'), { condominio: page.props.condominio.id }),
      filterParams.value,
      {
        preserveState: true,
        replace: true,
...
<template>
  <div class="flex items-center justify-between w-full mb-3">
    ...
    <Link
      :href="route(generateRoute('gestionale.tabelle.create'), { condominio: page.props.condominio.id })"
