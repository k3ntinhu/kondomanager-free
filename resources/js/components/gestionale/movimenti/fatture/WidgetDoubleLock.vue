...
const fiscalAssistant = computed(() => {
    ...
    if (semaforoContabile.value === 'ORANGE') {
        return {
            ...
            desc: trans('gestionale.fatture.double_lock.assistant.unbalanced_description', {
                amount: euro(scopertoAttualeEuro.value * 100),
            }),
            icon: Scale,
    ...
    if (semaforoContabile.value === 'RED') {
        return {
            ...
            desc: trans('gestionale.fatture.double_lock.assistant.structural_alert_description', {
                amount: euro(bucoRataZeroCents.value),
            }),
            icon: AlertOctagon,
                    ...
                    <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
                        ...
                        <div class="p-4 rounded-xl border flex flex-col justify-between" :class="bucoRataZeroCents > 0 ? 'bg-rose-50/50 border-rose-200' : 'bg-emerald-50/50 border-emerald-200'">
                            ...
                            <div v-if="bucoRataZeroCents > 0" class="text-[10px] font-bold text-rose-600 mt-3 leading-tight">
                                ⚠️ {{ trans('gestionale.fatture.double_lock.labels.missing_amount', { amount: euro(bucoRataZeroCents) }) }}
                            </div>
